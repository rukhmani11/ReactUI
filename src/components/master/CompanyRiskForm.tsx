...
const CompanyRiskForm = () => {
  ...
  const validate = (fieldValues: CompanyRiskModel = values) => {
    let temp: any = { ...errors };
  ...
  const { values, setValues, errors, setErrors, handleInputChange, resetForm } =
    useForm(companyRiskService.initialFieldValues, validate, id);

  const newcompanyRisks = () => {
    setValues(companyRiskService.initialFieldValues);
  };

  function setFormValue(model: CompanyRiskModel) {
    let newModel = {
  ...
  useEffect(() => {
    ...
    if (id) {
      getcompanies();
      setErrors({});
    } else newcompanyRisks();
  }, [id]);

  const getcompanies = () => {
    companyRiskService.getById(id).then((response) => {
      if (response) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validate()) {
      ...
      if (id) {
        companyRiskService.put(values).then((response: any) => {
          if (response) {
            let result = response.data;
            if (result.isSuccess) {
              globalService.success("Company Risk edited successfully.");
              resetForm();
              navigate("/companyRisk" + "/" + companyId);
            } else {
        ...
        companyRiskService.post(values).then((response: any) => {
          if (response) {
            let result = response.data;
            if (result.isSuccess) {
              globalService.success("Company Risk added successfully.");
              resetForm();
              navigate("/companyRisk" + "/" + companyId);
            } else {
              globalService.error(result.message);
            ...
            <CardActions sx={{ display: "flex", justifyContent: "center" }}>
              <Stack spacing={2} direction="row">
                ...
                <Button
                  ...
                  onClick={() => navigate("/companyRisk" + "/" + companyId)}
                >
                  Back To List
                </Button>
              </Stack>
